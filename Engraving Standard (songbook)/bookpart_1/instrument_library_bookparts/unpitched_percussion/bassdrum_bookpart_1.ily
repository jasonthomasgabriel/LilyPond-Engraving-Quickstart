...
%%%% bookpart_1/instrument_library_bookparts/unpitched_percussion/bassdrum_bookpart_1.ily
%%%% version: 3.1
...
\bookpart {
  \header {
    ...
    % subtitle        = ""
    % subsubtitle     = ""
    instrument        = "Bass Drum" % "Part" would be an apt name, as this could easily say "full score". I could change it, but the whole point is to leave as much Lilypond's standards as possible.
    % composer        = ""
    % arranger        = ""
    % poet            = ""
    % meter           = ""
  ...
  % Optionally set staff size in \layout{}, depending on needs when dealing with multiple scores.
    \score { \scoring_bassdrum_bookpart_one \layout { %{ #(layout-set-staff-size 20) %} indent = 0\mm short-indent = 0\mm } \midi { \set Score.midiChannelMapping = #'instrument } }
  
  % Articulated MIDI score block %
    % (Purposely creates no visual output) %
    \score { \unfoldRepeats \articulate << \scoring_bassdrum_bookpart_one >> \midi { \set Score.midiChannelMapping = #'instrument } }
}