...
%%%% bookpart_1/instrument_library_bookparts/_user_instruments/user_instrument_6_bookpart_1.ily
%%%% version: 3.1.2
...
\bookpart {
  ...
  % Optionally set staff size in \layout{}, depending on needs when dealing with multiple scores.
    \score { \scoring_user_instrument_six_bookpart_one \layout { %{ #(layout-set-staff-size 20) %} indent = 0\mm short-indent = 0\mm } \midi { \set Score.midiChannelMapping = #'instrument } }
  
  % Articulated MIDI score block %
    % (Purposely creates no visual output) %
    \score { \unfoldRepeats \articulate << \scoring_user_instrument_six_bookpart_one >> \midi { \set Score.midiChannelMapping = #'instrument } }
}