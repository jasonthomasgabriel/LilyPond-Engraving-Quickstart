...
%%%% music/parts/instrument_library/resources/books/pitched_percussion/book_crotales.ily
%%%% version: 4.0
...
\book {
  ...
  \bookpart {
    ...
    % Score 
      ...
      \score { \scoring_crotales_part \layout { %{ #(layout-set-staff-size 20) %} } \midi { \set Score.midiChannelMapping = #'instrument } }

    % Articulated MIDI score block %
      % (Purposely creates no visual output) %
      \score { \unfoldRepeats \articulate << \scoring_crotales_part >> \midi { \set Score.midiChannelMapping = #'instrument } }
  }
}