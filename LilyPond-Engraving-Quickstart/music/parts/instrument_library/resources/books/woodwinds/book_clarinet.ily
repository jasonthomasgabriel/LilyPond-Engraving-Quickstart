...
%%%% music/parts/instrument_library/resources/books/woodwinds/book_clarinet.ily
%%%% version: 4.0
...
\book {
  ...
  \bookpart {
    \header {
      ...
      instrument = \markup \concat { "Clarinet in B" \super \flat }
    }
    ...
    % Score 
      ...
      \score { \scoring_clarinet_part_transposed \layout { %{ #(layout-set-staff-size 20) %} } }
      
      % (MIDI output only - concert pitch)
      \score { \scoring_clarinet_part \midi { \set Score.midiChannelMapping = #'instrument } }

    % Articulated MIDI score block %
      % (Purposely creates no visual output) %
      \score { \unfoldRepeats \articulate << \scoring_clarinet_part >> \midi { \set Score.midiChannelMapping = #'instrument } }
  }
}