...
%%%% music/parts/instrument_library/resources/books/woodwinds/book_oboe.ily
%%%% version: 4.0
...
\book {
  ...
  \bookpart {
    ...
    % Score 
      ...
      \score { \scoring_oboe_part \layout { %{ #(layout-set-staff-size 20) %} } \midi { \set Score.midiChannelMapping = #'instrument } }

    % Articulated MIDI score block %
      % (Purposely creates no visual output) %
      \score { \unfoldRepeats \articulate << \scoring_oboe_part >> \midi { \set Score.midiChannelMapping = #'instrument } }
  }
}