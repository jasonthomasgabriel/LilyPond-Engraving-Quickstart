...
%%%% music/parts/instrument_library/resources/books/user_instruments/book_user_instrument_six.ily
%%%% version: 4.0
...
\book {
  \bookOutputSuffix "user_instrument_six"
  \bookpart {
    \header {
      ...
      instrument = \user_instrument_six_instrumentname
    }

    % Score 
      % Optionally set staff size in \layout{}, depending on needs when dealing with multiple scores.
      \score { \scoring_user_instrument_six_part \layout { %{ #(layout-set-staff-size 20) %} } \midi { \set Score.midiChannelMapping = #'instrument } }

    % Articulated MIDI score block %
      % (Purposely creates no visual output) %
      \score { \unfoldRepeats \articulate << \scoring_user_instrument_six_part >> \midi { \set Score.midiChannelMapping = #'instrument } }
  }
}