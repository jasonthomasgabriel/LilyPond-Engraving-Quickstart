...
%%%% music/parts/instrument_library/resources/books/voice/book_bass.ily
%%%% -----------------------------------------------------------
...
\book {
  ...
  \bookpart {
    ...
    % Score 
      ...
      \score { \scoring_bass_part \layout { %{ #(layout-set-staff-size 20) %} } \midi { \set Score.midiChannelMapping = #'instrument } }

    % Articulated MIDI score block %
      % (Purposely creates no visual output) %
      \score { \unfoldRepeats \articulate << \scoring_bass_part >> \midi { \set Score.midiChannelMapping = #'instrument } }
  }
}